...
var debug = require('debug')('js-pipeline.input.Socket.IO.Client');
var debug_events = require('debug')('js-pipeline.input.Socket.IO.Client:Events');
var debug_internals = require('debug')('js-pipeline.input.Socket.IO.Client:Internals');
...
module.exports = new Class({
  ...
  //exists: function(err, resp, body, req){
	...
	initialize: function(options){
		...
		this.profile('socket.io.client_init');//start profiling

		var first_connect = function(){
			debug_internals('first_connect %o', this.io);

			//console.log('Socket.IO.ClientPoll.ON_CONNECT');
			//console.log(result);
		...
		this.profile('socket.io.client_init');//end profiling

		this.log('socket.io.client', 'info', 'socket.io.client started');
  },
  // connect: function(){
	// 	// debug('this.connect');
  //   //
